...
class _UserUpdate extends State<Update>{
  ...
  Widget build(BuildContext context) {
    ...
    final logo =Center(child: Icon(Icons.edit ,size: 50),);
    ...
    return Scaffold(
      ...
      body:BlocListener<AuthBloc,AuthStates>(listener: (context,state){
        if(state is UserUpdateSuccessState ){
          ...
          Flushbar(
            title:  "Infos de compte modifié ",
            message:  "Vos infos ont été modifié Cher Client",
                ...
                Padding(padding: EdgeInsets.symmetric(vertical: 16.0),
                    child: RaisedButton(
                        ...
                        onPressed: (){if(_formKey.currentState.validate()){
                          blocA.add(UpdateButtonPressed(email: email.text,password: password.text,tel: tele.text,infos_liv: infos_liv.text,nom: nom.text));
                          Scaffold.of(context).showSnackBar(SnackBar(content: Text(' Infos Modifié ')));
